...
(train_i, train_l), (test_i, test_l) = datasets.cifar10.load_data()

train_i, test_i = train_i / 255.0, test_i / 255.0
train_l = utils.to_categorical(train_l)
test_l = utils.to_categorical(test_l)
...
class CIFARSequence(tf.keras.utils.Sequence):
    ...
    def __init__(self, data=None, batch_size=128):
        if data:
            ...
            images, labels = CIFARSequence.__generate_fake_data(
                num_images=128, num_classes=10)
        self.images, self.labels = CIFARSequence.__preprocessing(images, labels)
...
def main():
    ...
    heldout_seq = CIFARSequence(data=(test_i, test_l), batch_size=batch_size)
    ...
    def create_model_bay():
        ...
        kl_divergence_fn = (lambda q, p, _: tfd.kl_divergence(q, p) / tf.cast(len(train_i), dtype=tf.float32))
    ...
    model_det.load_weights('./model_weights/cifar_det').expect_partial()
    model_bay.load_weights('./model_weights/cifar_bay').expect_partial()


    def det_batch_spsa(batch_number=0, n_images=batch_size, thresholds=[0.0], figures=0, epsilon=0.1):
        test_images, test_labels = CIFARSequence.__getitem__(heldout_seq, batch_number)
        ...
        for i in range(min(n_images, figures)):
            ...
            plt.ylim([0, 1])
            plt.savefig(f"./images_cifar/{data_type}_{i}_prediction.jpeg", bbox_inches='tight')
            plt.clf()
    ...
    def bay_batch_spsa_fb(batch_number=0, n_images=batch_size, thresholds=[0.0], figures=0, iterations=100, epsilon=0.1):
        ...
        for i in range(iterations):
            for j in range(n_images):
                prediction = model_bay.predict(spsas[j])
                for k in range(10):
